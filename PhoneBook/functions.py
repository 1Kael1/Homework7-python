...
def ExportContactsInCSV(path, file_name, contacts):
    with open(path + file_name, 'w') as file:
        ...
        for i in contacts:
            file.writelines(
                f"{contacts[i]['First Name']};{contacts[i]['Last Name']};{i};{contacts[i]['Description']}\n")
...
def SaveData(phonebook):
    with open('Homework7\PhoneBook\PBdata.txt', 'w') as file:
        for key in phonebook:
            file.write(
                f"{phonebook[key]['First Name']};{phonebook[key]['Last Name']};{key};{phonebook[key]['Description']}\n")
    ...
    for key in new_phones:
        if key in phonebook:
            if key in phonebook and overwrite != 'y+' and overwrite != 'n+':
                overwrite = view.get_user_changes_option(
                    key, phonebook[key]['First Name'], phonebook[key]['Last Name']).lower()
